...
def run_scenario(path):
    with open(path) as f: sc=yaml.safe_load(f)
    return {'name': sc.get('name'), 'checks': sc.get('checks', []), 'actions': sc.get('actions', [])}
if __name__=='__main__':
    ap=argparse.ArgumentParser(); ap.add_argument('--scenario', required=True); ap.add_argument('--out', default='artifacts/scenario.json')
